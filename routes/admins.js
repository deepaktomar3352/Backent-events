...
router.post("/chkadminlogin",(req,res)=>{
    ...
    else
    {
      if(result.length==0)
      {
        res.status(500).json({status:false,message:'Invalid emailadress/mobilenumber/password'})
      }
      else
      {
        console.log(result)
        res.status(200).json({status:true,data:result,message:'your are signIn'})
      }
    }
    })
});
...
router.post("/studentchkadminlogin",(req,res)=>{
    console.log(req.body.emailaddress)
    console.log(req.body.password)
    pool.query("select * from student_registration where (student_email=? or student_mobile=?) and student_password=?",[req.body.emailaddress,req.body.emailaddress,req.body.password],(error,result)=>{
        if(error)
    {
      res.status(500).json({status:false,message:'server error'})
    }
    else
    {
      ...
      else
      {
        ...
        res.status(200).json({status:true,data:result,message:"welcome you are Sign In"})
      }
    }
    })
});
